...
MIDIMultiTrack tracks = MIDIMultiTrack(17);
MIDISequencerGUINotifierText notifier;
//AdvancedSequencer sequencer(&notifier);         // an AdvancedSequencer (without GUI notifier)
AdvancedRecorder recorder(&tracks, &notifier);      // an AdvancedRecorder recording tracks of the sequencer
...
int main( int argc, char **argv ) {
    ...
    while (command != "quit") {                     // main loop
        ...
        if(command == "load") {                   // loads a file
            if (LoadMIDIFile(par1.c_str(), &tracks))
            ...
            recorder.Start();
            cout << "Recorder started at measure: " << recorder.GetCurrentMeasure() << ":"
                 << recorder.GetCurrentBeat() << endl;
        }
        else if (command == "loop") {               // sets repeated play
            int beg = atoi(par1.c_str());
            int end = atoi(par2.c_str());
            if (!(beg == 0 && end == 0)) {
                recorder.SetRepeatPlay(true, beg, end);
                cout << "Repeat play set from measure " << beg << " to measure " << end << endl;
            }
            else {
                recorder.SetRepeatPlay(false, 0, 0);
                cout << "Repeat play cleared" << endl;
            }
        }
        else if (command == "stop") {               // stops playback
            recorder.Stop();
            cout << "Recorder stopped at measure: " << recorder.GetCurrentMeasure() << ":"
                 << recorder.GetCurrentBeat() << endl;
        }
            ...
            if (recorder.GoToMeasure(measure, beat))
                cout << "Actual position: " << recorder.GetCurrentMeasure() << ":"
                     << recorder.GetCurrentBeat() << endl;
            else
            ...
            if (port < 0 || (unsigned)port >= MIDIManager::GetNumMIDIOuts())
                ...
                recorder.SetTrackOutPort(track, port);
                cout << "Assigned out port n. " << recorder.GetTrackPort(track)
                     << " to track " << track << endl;
            }
        }
        else if (command == "tscale") {             // scales playback tempo
            int scale = atoi(par1.c_str());
            recorder.SetTempoScale(scale);
            cout << "Tempo scale : " << scale << "%  " <<
                    " Effective tempo: " << recorder.GetTempoWithScale() << " bpm" << endl;
        }
        else if (command == "tshift") {             // sets the time shift (in ticks) of a track
            int track = atoi(par1.c_str());
            int amount = atoi(par2.c_str());
            recorder.SetTrackTimeShift(track, amount);
            ...
            for (unsigned int i = 0; i < tracks.GetNumTracks(); i++) {
                ...
                cout << "Track " << i << ": " << recorder.GetState()->track_states[i]->track_name << endl;
                if (trk->IsEmpty())
                    cout << "EMPTY" << endl;
                else {
            ...
            int meas = recorder.GetCurrentMeasure();
            if (meas > 0)
                recorder.GoToMeasure(--meas);
            cout << "Actual position: " << recorder.GetCurrentMeasure() << ":"
                 << recorder.GetCurrentBeat() << endl;
        }
        else if (command == "f") {                  // goes a measure forward
            int meas = recorder.GetCurrentMeasure();
            if (recorder.GoToMeasure(++meas))
                cout << "Actual position: " << recorder.GetCurrentMeasure() << ":"
                     << recorder.GetCurrentBeat() << endl;
        }
